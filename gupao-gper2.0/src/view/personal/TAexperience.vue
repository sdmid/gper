<template>
  <div class="person-nav">
    ...
    <div class="person-content">
      <div class="time-item" v-for="(item,index) in pageQueryByCreditsData.list">
        <div class="time-left">
          <span v-if="item.action === 'punish_user'">{{ ' - ' + item.credits}}</span>
          <span v-else>{{item.credits > 0 ? ' + ' + item.credits : item.credits}}</span>
        </div>
        <div class="time-right ">
          <span class="hot_title">{{item.action | getCoinAction}} ·

            <router-link v-if="item.action == 'create_article'" class='hover_blue' :to="{path:'/articleContent',query:{id:item.sourceId}}">
              {{item.sourceSubject}} ·
            </router-link>
            <router-link v-if="item.action !== 'create_article' && item.sourceSubject" class='hover_blue' :to="{name:'AnswerDetail',params:{id: item.sourceId}}">
              {{item.sourceSubject}} ·
            </router-link>
      ...
      <div class="answers_page" v-show="pageQueryByCreditsData.total > 0" >
        <Page :total="pageQueryByCreditsData.total" @on-change="handlePage" />
      </div>
    </div>
  </div>
...
<script>
  ...
  export default{
    ...
    mounted(){
      ...
      this.$store.state.person.pageQueryByCreditsParmas.page = 1;
      this.$store.dispatch('pageQueryByCredits',this.params.userid);
    },
    methods:{
      handlePage(i){
        this.$store.state.person.pageQueryByCreditsParmas.page = i;
        this.$store.dispatch('pageQueryByCredits',this.params.userid);
      }
    },
    computed:{
      pageQueryByCreditsData(){
        return this.$store.state.person.pageQueryByCreditsData;
      },
      pageQueryByCreditsParmas(){
        return this.$store.state.person.pageQueryByCreditsParmas;
      }
    },
    filters:{
